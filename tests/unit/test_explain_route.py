"""Unit test for POST /capa/explain's plumbing (cache hit/miss, audit write,
404/422/502) — mocks evaluator.run/explainability.run/get_repository/
get_context_package so no network or DB is touched. See phases/phase6.md."""
...
from models.schemas import CapaAction, ContextPackage, DimensionResult, EvaluationResult, RecurrenceResult  # noqa: E402
...
def _capa_action() -> CapaAction:
    return CapaAction.model_validate(
        dict(
            action_id="ACT_001",
            capa_id="CAPA_EXPLAIN_ROUTE_TEST",
...
def client(monkeypatch):
    ...
    async def fake_explainability_run(eval_result, scoring_result, action_text):
        return "Plain-language explanation."

    monkeypatch.setattr(dispatch_mod.evaluator, "run", fake_evaluator_run)
    monkeypatch.setattr(dispatch_mod.explainability, "run", fake_explainability_run)
...
def test_explain_cache_miss_runs_evaluator_and_populates_cache(client):
    resp = client.post("/capa/explain", json=_capa_payload(action_id="ACT_001"))
    assert resp.status_code == 200
    body = resp.json()
    assert body["explanation"] == "Plain-language explanation."
    assert body["weakness_level"] == "None"
    assert client._test_state["evaluator_calls"] == 1
    assert len(client._fake_repo.audit_entries) == 1
    assert client._fake_repo.audit_entries[0].agent == "explainability"

    key = eval_cache.make_key("TENANT_ACERTECH", "CAPA_EXPLAIN_ROUTE_TEST", "ACT_001", "")
    assert eval_cache.get(key) is not None


def test_explain_cache_hit_skips_evaluator(client):
    key = eval_cache.make_key(
        "TENANT_ACERTECH", "CAPA_EXPLAIN_ROUTE_TEST", "ACT_001",
...
def test_explain_unknown_action_id_returns_404(client):
    resp = client.post("/capa/explain", json=_capa_payload(action_id="ACT_DOES_NOT_EXIST"))
    assert resp.status_code == 404


def test_explain_requires_action_id_or_actions(client):
    resp = client.post("/capa/explain", json=_capa_payload())
    assert resp.status_code == 422


def test_explain_502_only_from_eval_resolution_not_agent(client, monkeypatch):
    async def failing_evaluator_run(agent_input, repo):
        raise ValueError("Evaluator failed: boom")

    monkeypatch.setattr(dispatch_mod.evaluator, "run", failing_evaluator_run)
    resp = client.post("/capa/explain", json=_capa_payload(action_id="ACT_001"))
    assert resp.status_code == 502


def test_explain_ad_hoc_text_works(client):
    resp = client.post("/capa/explain", json=_capa_payload(actions=["Train the team."]))
    assert resp.status_code == 200
    assert len(client._fake_repo.audit_entries) == 1
